...
function App() {
  ...
  const filteredAgents = agents.filter(agent => {
    const matchesText = agent.displayName.toLowerCase().includes(searchText.toLowerCase());
  ...
  useEffect(() => {
    setCurrentPage(1); // Reinicia a la primera página si los filtros cambian
  ...
  return (
    <div className="container">
      ...
      <div className="filters">
        ...
        <button className="view-team-button" onClick={() => setIsModalOpen(true)}>Ver Equipo ({team.length})</button>
      </div>
        ...
        {currentAgents.length > 0 ? (
          currentAgents.map(agent => (
            <div key={agent.uuid} className="agent-card">
              ...
              <button onClick={() => addToTeam(agent)} className="add-to-team-button">Agregar al equipo</button>
            </div>
          ))
        ) : (
      ...
      {isModalOpen && <TeamModal team={team} onClose={() => setIsModalOpen(false)} />}
    </div>
  );
}
